...
exports.addVehicle = async (req, res) => {
    try {
        ...
        // Create new vehicle
        const newVehicle = new Vehicle({ owner, manufacturer, model, year, licensePlate });
        await newVehicle.save();

        // Update user's vehicles array
        await User.findByIdAndUpdate(owner, { $push: { vehicles: newVehicle._id } });
...
exports.updateVehicle = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: 'Vehicle information updated successfully', vehicle });
    } catch (error) {
